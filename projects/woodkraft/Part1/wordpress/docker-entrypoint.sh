...
echo "Get old host from db"
old_host=$(wp --allow-root option get siteurl)

echo "Replacing old host ${old_host} with ${WP_HOST} in DB"
wp --allow-root search-replace ${old_host} ${WP_HOST}

# Rename site
echo "rename site url to ${WP_HOST} in wp-config.php"
sed -i -e "s!define('WP_HOME'.*!define('WP_HOME','${WP_HOST}');!" /var/www/html/wp-config.php
sed -i -e "s!define('WP_SITEURL'.*!define('WP_SITEURL','${WP_HOST}');!" /var/www/html/wp-config.php
